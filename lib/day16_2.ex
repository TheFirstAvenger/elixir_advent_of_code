defmodule AdventOfCode.Day16_2 do
  ...
  def check_mfcsam([{hi,hc}|t], i, aunts) when hi in ["cats","trees"] do
    case Map.get(aunts, "#{i}:#{hi}") do
      x when x > hc -> check_mfcsam(t, i, aunts)
      nil -> check_mfcsam(t, i, aunts)
      _   -> false
    end
  end

  def check_mfcsam([{hi,hc}|t], i, aunts) when hi in ["pomeranians","goldfish"] do
    case Map.get(aunts, "#{i}:#{hi}") do
      x when x < hc -> check_mfcsam(t, i, aunts)
      nil -> check_mfcsam(t, i, aunts)
      _   -> false
